...
template <typename T>
static int bintree<T>::leftDepth(node<T>* leaf) {
    int depth = 0;
...
template <typename T>
static int bintree<T>::rightDepth(node<T>* leaf) {
    int depth = 0;
...
template <typename T>
static int bintree<T>::depth(node<T>* leaf) {
    if (leaf == nullptr)
        return 0;
...
template <typename T>
static int bintree<T>::getMaxWidth(node<T>* leaf) {
    int maxWidth = 0;
    int width;
    int h = depth(leaf);
    int i;
...
template<typename T>
static int bintree<T>::getWidth(node<T>* leaf, int level) {
    if (leaf == nullptr)
        return 0;
